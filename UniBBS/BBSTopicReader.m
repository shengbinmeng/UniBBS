...
{    
    ...
    if (returnedData) {
        ...
        while (range.length > 0) {
            ...
            NSRange tmpRange = [m rangeAtIndex:1];
            if (m && tmpRange.length) {
                NSString *str = [pageSource substringWithRange:tmpRange];
            ...
            tmpRange = [m rangeAtIndex:1];
            if (m && tmpRange.length) {
                NSString *str = [pageSource substringWithRange:[m rangeAtIndex:1]];
                NSString *address = [NSString stringWithFormat:@"bbspsm.php?board=%@", str];
